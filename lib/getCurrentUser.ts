 ...
 * - In Route Handlers, pass `req` and we read the already-verified user headers set by `middleware.ts`.
...
export async function getCurrentUser(req?: Request): Promise<TokenPayload | null> {
  if (req) {
    ...
    // Fallback if middleware didn't run (or headers missing): verify from cookie header
